...
RULES = [
    {
        ...
        "severity": "Критично",
        "score": 8,
        "title": "Есть признаки reverse shell",
        "summary": "код готовит соединение наружу и рядом с ним использует оболочку или subprocess",
        "advice": "убрать сетевой shell-контур и любые вызовы командной оболочки",
    ...
    },
    {
        ...
        "summary": "код вызывает shell, subprocess или os.system и может запускать внешние команды",
        "advice": "убрать прямые shell-вызовы или жёстко ограничить допустимые команды",
        "patterns": [
            r"os\.system\s*\(",
...
COMBOS = [
    ...
    },
    {
        ...
        "summary": "сетевой слой сочетается с динамическим выполнением кода, что уже выглядит как loader-цепочка",
    ...
    },
    {
        ...
        "summary": "автозапуск сочетается с shell-командами, что похоже на механизм закрепления payload",
...
def _detect_family(code: str) -> dict:
    ...
    if extera_score == 0 and hikka_score == 0:
        return {"family": "generic", "label": "Python script", "reasons": []}
...
class NeuralVMod(loader.Module):
    ...
    async def vcheck(self, message):
        ...
        try:
            ...
            return await utils.answer(m_status, f"<b>❗️ NeuralV:</b> Не удалось прочитать файл: <code>{_escape_html(str(e))}</code>")
